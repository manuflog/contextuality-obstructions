...
| **Canonical MASA-stack class, order exactly n** | **proved (algebra) / under review (geometry)** | equivariant-hom lemma proved + `thmG_general.py`; full stack-cohomology theorem **not** independently certified | Paper A | **largest open item**: the stack/cohomology layer needs a specialist proof audit |
