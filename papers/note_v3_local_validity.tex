...
outcome while $E_1$ is non-projective. Dimension three is necessary: repeatability on a
nonzero outcome forces its effect to have an eigenvalue-$1$ sharp core (V40), so at $d=2$ a
two-outcome POVM with both outcomes repeatable has two orthogonal rank-one cores summing to
the identity, hence is projective. The qutrit witness escapes this because the third
dimension carries the shared weight $c$, $1-c$ without either core losing its eigenvalue $1$.
\end{theorem}
