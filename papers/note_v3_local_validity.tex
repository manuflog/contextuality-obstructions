...
forced, not optional. Evidence (V48): $5{,}556$ state--family records at $d=4$ and
$d=8$ (all six deletions of the certified family, double deletions, boundary
bisections to $10^{-6}$, and fresh anomalous magic squares including odd order-$8$
phases) with zero mismatches between the $\CF$ LP and hull membership, cross-arbitrated
by the exact facet census. The residual gap to a proof is a completion lemma reducing
the full per-context character vector to the paired coordinates --- the general-family
analogue of the on-hull lemma (Prop.~\ref{lem:onhull}).
...
$d$ the $\mu_{2d}$ bound is section-dependent --- the DFT section violates it at
$d\equiv 3\pmod 4$ (pinned: the $d=3$ observer loop has holonomy $e^{i\pi/6}$), while
the Weil-normalized section restores it (verified $d=3,5,7$; the general odd-$d$ proof
via the Gauss-sum determinant remains open). Claims must pin the section --- the same
...
(V49): support-presheaf and scenario-complex cohomologies provably cannot capture it
(pinned no-gos); on the pentagon a $\U(1)$-holonomy-twisted spectral pairing on the
non-orthogonality graph characterizes the state sector exactly, with the $(-1)$-twist
landing on the classical bound --- the general formulation is open. (b) The
general-$d$ facet census and the arithmetic profile of the $61$ classes. (c)
Conjecture~\ref{conj:pair} (the residual completion lemma) and the general odd-$d$
Weil-section proof for Theorem~\ref{conj:tower}.
\end{openproblem}
...
Thm.~\ref{conj:tower} & this note & \texttt{tower\_holonomy.py} (V47)\\
Conj.~\ref{conj:pair} (evidence) & this note; \cite{paperC} & \texttt{scenario\_paired\_evidence.py} (V48)\\
