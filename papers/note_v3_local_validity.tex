% Local Validity and Contextual Holonomy
% Preprint source.
...
\title{Local Validity and Contextual Holonomy:\\ A Structural Formulation of the Copenhagen Principle}
\author{Manuel Flores Gordillo\thanks{Independent researcher. ORCID
\texttt{0009-0006-8246-0343}; \texttt{manuel.flores@columbia.edu}.
Verification scripts, data, and companion papers:
...
now proved for closed-triple Weyl families at even $d$ --- neither hypothesis is decorative),
with the
achievable state-independent values classified (the value classification --- even/odd dichotomy and
fixed value $d/2$ --- is due to Abramsky, Cercelescu \& Constantin, FSCD 2024 \cite{ACC}, and is recast here in
...
the public suite, or is explicitly labeled a conjecture.
\end{abstract}
...
stated as an explicit conjecture with its current evidence. The hardware measurements
reported in \S\ref{sec:observer} are the one exception to that trichotomy and are given
...
holonomy along closed context loops is a quantized obstruction, and for closed-triple Weyl
families at even $d$ this obstruction \emph{is} the all-versus-nothing sector of
contextuality, with the state sector governed by an enumerable facet geometry. \emph{(iii) Collapse.} Statistics, repeatability, and covariance under the measured
...
$\omega$-powers, and integrality is automatic rather than a gauge choice. Genuine
$\tau$-steps are an odd-$d$ phenomenon. The \emph{state-sector} $\tau$-necessity results
of \cite{paperC} concern a different object and are not in tension with this.
Explicit per-context trivializers: \texttt{local\_classicality.py} (V23),
...
this even/odd dichotomy and the fixed value $d/2$ are \textbf{due to Abramsky, Cercelescu
\& Constantin} \cite[Thm.~16]{ACC}; \cite[Thm.~1]{paperB} is a re-derivation in
Weyl-certificate language, not an independent result and not a strengthening. The
commutator-carry mechanism behind (c) is likewise theirs \cite[Lem.~15, Thm.~16]{ACC}.
\end{theorem}
\begin{proof}
...
\cite[Thm.~4]{paperB}, whose mechanism is that of \cite[Lem.~15]{ACC}. Finally, no gauge hypothesis is needed: by the parity lemma of
...
\texttt{solvability\_equivalence.py} (V44),
\texttt{tau\_gauge\_general.py} (V45, including the $d=6$ extension), and
\texttt{mu\_d\_valuation.py} (V46).
\end{proof}
The converse is thus a corollary of \cite{paperB}'s proven theorems plus a few lines of
ring theory. It is also exercised over $2{,}100$ machine-checked families
(\texttt{holonomy\_vs\_solvability.py}); that sweep is a consistency check on the
implementation rather than a search that could have failed, since only three of the
$1{,}500$ families at $d=2$ are unsolvable and none of the $600$ at $d=4$ are, so it
contains almost no positive instances and carries correspondingly little weight.
...
following results replace any loose appeal to ``holonomy of the state'' with exact
geometry.
...
end-to-end via a completion lemma and Abramsky--Barbosa--Mansfield duality \cite{ABM}.
\end{theorem}
...
$\mu_2$ otherwise, exhaustive only at $d=3$. Claims must therefore pin the section: a holonomy statement that
leaves the section unspecified is not yet a statement.
...
\begin{theorem}[Sharp core, and the efficiency dichotomy; V40, V56]\label{thm:sharp}
\emph{(Known.)} For a POVM instrument, statistics plus repeatability force every output of
$\mathcal I_j$ into the eigenvalue-$1$ subspace of its effect $E_j$, and these subspaces are
mutually orthogonal; a strictly unsharp effect (no $1$-eigenvector) admits no repeatable
instrument at all. \emph{(New here.)} Whether the folk claim ``repeatable $\Rightarrow$
projective'' holds is decided by \emph{efficiency}:
\begin{enumerate}[label=(\alph*),topsep=2pt,itemsep=1pt]
\item If $\mathcal I_1(\rho)=K\rho K^{\dagger}$ is efficient (single Kraus operator) and
repeatable on its outcome, then $E_1$ \emph{is} a projection, in every finite dimension.
\item Without efficiency the claim is false already at $d=3$, and $d=3$ is minimal: the qutrit
two-effect POVM $E_1=\mathrm{diag}(1,0,c)$, $E_2=\mathrm{diag}(0,1,1-c)$, $0<c<1$, admits a
repeatable covariant instrument on \emph{every} outcome while $E_1$ is non-projective, whereas at
$d=2$ two orthogonal eigenvalue-$1$ cores summing to the identity force projectivity.
\end{enumerate}
\end{theorem}
\begin{proof}
(a) Efficiency and the statistics condition give $K^{\dagger}K=E_1$; repeatability gives
$\operatorname{tr}(E_1K\rho K^{\dagger})=\operatorname{tr}(E_1\rho)$ for all $\rho$, i.e.
$K^{\dagger}E_1K=K^{\dagger}K$, so $K^{\dagger}(I-E_1)K=0$. Writing $I-E_1=S^{\dagger}S$ this
reads $(SK)^{\dagger}(SK)=0$, hence $SK=0$ and $\operatorname{ran}K\subseteq\ker(I-E_1)$, the
eigenvalue-$1$ eigenspace of $E_1$. Polar decomposition $K=U|K|$ with $|K|=E_1^{1/2}$ gives
$\dim\operatorname{ran}K=\operatorname{rank}E_1$, so $\operatorname{rank}E_1\le\dim\ker(I-E_1)$;
the reverse inequality always holds, so $\operatorname{ran}E_1=\ker(I-E_1)$ and $E_1$ is the
orthogonal projector onto it. (b) The qutrit instrument
$\mathcal I_j(\rho)=\operatorname{tr}(E_j\rho)\,|e_j\rangle\langle e_j|$, $j=1,2$, is repeatable
and covariant; it needs $\operatorname{rank}E_1=2$ Kraus operators, so it is not efficient, in
accordance with (a). At $d=2$ the two eigenvalue-$1$ cores are orthogonal and each nonzero,
hence one-dimensional and spanning, so $E_1$ is the projector onto its own core.
\end{proof}
The known half is the norm-1 property of the measured observable: repeatability confines the
posterior states to the unit-eigenvalue subspaces, which is Ozawa's framework \cite{Ozawa84} (where
it also yields that repeatable measurements force discrete spectrum), developed systematically by
Busch, Grabowski and Lahti \cite{BGL95}, named and studied as the norm-1 property by Heinonen,
Lahti, Pellonp\"a\"a, Pulmannov\'a and Ylinen \cite{HLPPY03}, and consolidated in \cite{BLPY16}.
In finite dimensions ``norm $1$'' is exactly ``has an eigenvalue-$1$ eigenvector'', so
Theorem~\ref{thm:sharp}'s first sentence is that literature specialized to finite $d$ and is
\emph{not} claimed as new. Part (a) is likewise the finite-dimensional content of
Buscemi, D'Ariano and Perinotti \cite{BDP04}, who exhibit nonorthogonal perfectly repeatable
measurements and show they occur only in infinite dimensions --- their analysis is carried out for
\emph{pure} measurements, one contraction $M_e$ per outcome, which is precisely efficiency, so (a)
is the reason their finite-dimensional conclusion holds and (b) is the reason it does not extend to
general instruments. What is offered here as new is the packaging: the efficiency dichotomy stated
as such, the covariance of the witness, and the minimality of $d=3$. Efficiency is the same hinge
that singles out L\"uders in Theorem~\ref{thm:lueders}, which is the point of stating it this way.
Certificate: \texttt{repeatable\_efficiency\_dichotomy.py} (V56).
...
Theorems~\ref{thm:lueders}--\ref{thm:twin} give the Leifer--Spekkens thesis \cite{LS} a sharpened
...
\end{theorem}
Theorem~\ref{thm:fr} \emph{refutes} the natural reading --- one this program held for a
time --- that the FR argument is a holonomy phenomenon: the switch holonomy
...
(\texttt{hardware/results/ibm\_fez\_holonomy\_20260709.json}). \textbf{Both figures are
shot-noise statistical only}, computed under the implemented witness model and conditional
on compatibility and nondisturbance. They are not systematics-aware; calibration drift and
look-elsewhere effects are not folded in, and neither run is loophole-free. The stored pubs
carry no per-observable labels, so the central compatibility diagnostic cannot be recomputed
from this data (\texttt{hardware/COMPATIBILITY.md}). Read them as consistency checks on real
hardware, not as precision measurements. Note also that $-i$ is a determinant-section
quantity: by Theorem~\ref{thm:fr} the ray-level holonomy is $+1$ gauge-invariantly, so an
interferometer measuring a Pancharatnam phase could not have registered $-i$ in any case.
The $56\sigma$ figure confirms the predicted ray-level value; it is not an empirical
discrimination against a live alternative.
...
moduli; rigidity $+$ forced torsion $\Leftrightarrow$ AvN) remains conjectural. Since this
note, the rigidity sub-program has been carried substantially further: exact rigidity
certificates have been produced across the known Kochen--Specker census, assembled into a
graded flex dictionary, and analyzed via the Peres--Penrose circle, published as a flagship
paper \cite{rigidity24}. This settles the certificate-production question across the census
but not the conceptual one posed here: the specific pairing conjectured above (flex
$\Leftrightarrow$ AvN/forced-torsion) is not thereby resolved and remains open exactly as
stated. (b)
The general-$d$ facet census. The $d=4$ arithmetic profile is complete (V53): $18$
species, all facet functionals rational --- every tier-2 functional admits an exact
$\{0,\pm1\}$ lift with integer bound in $\{6,7\}$, so no irrational unit bound occurs ---
and $\tau$-level data necessary for all $61$ classes; the general-$d$ census
...
All verification scripts named in the text, together with their pinned expected
output, are in the public repository \cite{repo}; the index is
\texttt{verification/INDEX.md}.

\section*{Use of AI tools}
Large-language-model assistants (Anthropic Claude) were used substantially in this
work: drafting and revising the manuscript, writing and debugging the verification
scripts, running literature searches, and proposing proof strategies. Every mathematical
claim above is either a theorem of the companion papers, a machine-verified certificate in
the public suite, or explicitly labeled a conjecture or open problem; the hardware results
are the one exception and are reported with their stated assumptions. AI-proposed
claims that failed verification were discarded rather than retained, and the author
takes full responsibility for the final text and results.
...
Thm.~\ref{thm:avn} & \cite{ACC} Thm.~16 (values); \cite{paperB} Thm.~1, 4 & \texttt{solvability\_equivalence.py} (V44); \texttt{tau\_gauge\_general.py} (V45)\\
...
Thm.~\ref{thm:sharp} (sharp core) & \cite{Ozawa84,BGL95,HLPPY03,BLPY16} & \texttt{s4\_povm\_bayes.py} (V40)\\
Thm.~\ref{thm:sharp}(a) & \cite{BDP04}, finite-$d$ form & \texttt{repeatable\_efficiency\_dichotomy.py} (V56)\\
Thm.~\ref{thm:sharp}(b), Thm.~\ref{thm:twin} & this note & \texttt{s4\_povm\_bayes.py} (V40); V56\\
...
S4/S5 proofs & supplement & \texttt{note\_v2\_patch.md} (\S S5's qubit example is superseded by Thm.~\ref{thm:sharp})\\
hardware (shot-noise only) & measurement & \texttt{ibm\_fez} job \texttt{d986q62f47jc73a7hm2g}\\
\bottomrule
\end{tabular}
\end{center}
...
This preprint belongs to a single research program; its companion papers are
listed below. Each identifier is a Zenodo \emph{concept} DOI, which always
resolves to the current version of the record it names.

\noindent\emph{Rigidity subseries.}
\begin{itemize}
\item M.~Flores Gordillo, \emph{Exact rigidity certificates for Kochen--Specker sets, the graded flex dictionary, and the Peres--Penrose circle}, Zenodo (2026), doi:10.5281/zenodo.21488473.
\item M.~Flores Gordillo, \emph{Geometry and arithmetic of the Kochen--Specker circle}, Zenodo (2026), doi:10.5281/zenodo.21498342.
\item M.~Flores Gordillo, \emph{The alphabet theorems: cancellation mechanisms, mechanism moduli, and the unique flexible family of two-symbol Kochen--Specker sets}, Zenodo (2026), doi:10.5281/zenodo.21498385.
\item M.~Flores Gordillo, \emph{A second Kochen--Specker circle, in dimension four}, Zenodo (2026), doi:10.5281/zenodo.21516711.
\item M.~Flores Gordillo, \emph{A tower of Kochen--Specker circles: even-dimensional unimodular families and the maximal-Galois holonomy law}, Zenodo (2026), doi:10.5281/zenodo.21680457.
\end{itemize}

\noindent\emph{Cohomology subseries.}
\begin{itemize}
\item M.~Flores Gordillo, \emph{The canonical class of the MASA context stack and the Peres--Mermin obstruction}, Zenodo (2026), doi:10.5281/zenodo.21630431.
\item M.~Flores Gordillo, \emph{The obstruction spectrum of Weyl context families and the 2-adic tower}, Zenodo (2026), doi:10.5281/zenodo.21630630.
\item M.~Flores Gordillo, \emph{What contextual holonomy detects}, Zenodo (2026), doi:10.5281/zenodo.21633744.
\item M.~Flores Gordillo, \emph{The Observer-Context Category: Relative Facts, a $\mathbb{Z}_2$ Switching Invariant, and the Movable Cut}, Zenodo (2026), doi:10.5281/zenodo.21630697.
\end{itemize}

\noindent\emph{Program record.}
\begin{itemize}
\item M.~Flores Gordillo, \emph{Contextuality as cohomological obstructions --- research program and verifications}, Zenodo (2026), doi:10.5281/zenodo.21498498.
\end{itemize}

\begin{thebibliography}{9}
\bibitem{ACC} S.~Abramsky, \c{S}.-I.~Cercelescu, C.~Constantin, \emph{Commutation Groups and
State-Independent Contextuality}, in \emph{9th International Conference on Formal Structures for
Computation and Deduction (FSCD 2024)}, LIPIcs \textbf{299}; extended version arXiv:2603.12197
(2026).
\bibitem{ABM} S.~Abramsky, R.~S.~Barbosa, S.~Mansfield, \emph{Contextual fraction as a measure of
contextuality}, Phys.\ Rev.\ Lett.\ \textbf{119}, 050504 (2017).
\bibitem{Ozawa84} M.~Ozawa, \emph{Quantum measuring processes of continuous observables},
J.\ Math.\ Phys.\ \textbf{25}, 79--87 (1984), doi:10.1063/1.526000.
\bibitem{BGL95} P.~Busch, M.~Grabowski, P.~J.~Lahti, \emph{Repeatable measurements in quantum
theory: their role and feasibility}, Found.\ Phys.\ \textbf{25}, 1239--1266 (1995),
doi:10.1007/BF02055331.
\bibitem{HLPPY03} T.~Heinonen, P.~Lahti, J.-P.~Pellonp\"a\"a, S.~Pulmannov\'a, K.~Ylinen,
\emph{The norm-1-property of a quantum observable}, J.\ Math.\ Phys.\ \textbf{44}, 1998--2008
(2003), doi:10.1063/1.1566454; arXiv:quant-ph/0212091.
\bibitem{BLPY16} P.~Busch, P.~Lahti, J.-P.~Pellonp\"a\"a, K.~Ylinen, \emph{Quantum Measurement},
Theoretical and Mathematical Physics, Springer (2016), doi:10.1007/978-3-319-43389-9.
\bibitem{BDP04} F.~Buscemi, G.~M.~D'Ariano, P.~Perinotti, \emph{There exist nonorthogonal quantum
measurements that are perfectly repeatable}, Phys.\ Rev.\ Lett.\ \textbf{92}, 070403 (2004),
doi:10.1103/PhysRevLett.92.070403; arXiv:quant-ph/0310041.
\bibitem{LS} M.~S.~Leifer, R.~W.~Spekkens, \emph{Towards a formulation of quantum theory as a
causally neutral theory of Bayesian inference}, Phys.\ Rev.\ A \textbf{88}, 052130 (2013).
\bibitem{rigidity24} M.~Flores Gordillo, \emph{Exact rigidity certificates for Kochen--Specker
sets, the graded flex dictionary, and the Peres--Penrose circle}, Zenodo (2026),
doi:10.5281/zenodo.21488473.
\bibitem{paperA} M.~Flores Gordillo, \emph{The Canonical Class of the MASA Context Stack and the Peres--Mermin Obstruction}, Zenodo (2026), doi:10.5281/zenodo.21630431.
\bibitem{paperB} M.~Flores Gordillo, \emph{The Obstruction Spectrum of Weyl Context Families and the 2-adic Tower}, Zenodo (2026), doi:10.5281/zenodo.21630630.
\bibitem{paperC} M.~Flores Gordillo, \emph{What Contextual Holonomy Detects}, Zenodo (2026), doi:10.5281/zenodo.21633744.
\bibitem{paperD} M.~Flores Gordillo, \emph{The Observer-Context Category: Relative
Facts, a $\mathbb Z_2$ Switching Invariant, and the Movable Cut}, Zenodo (2026),
doi:10.5281/zenodo.21630697.
