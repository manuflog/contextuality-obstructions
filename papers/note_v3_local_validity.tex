...
% Prepared by Claude for Manuel Flores Gordillo's adversarial review.
...
enumerable facet geometry. \emph{(iii) Collapse.} Statistics, repeatability, and covariance pin the
...
coboundary, with an explicit trivializing cochain
$\lambda$ taking values in the cyclotomic group $\mu_{4d}$; the half-steps
$\tau=e^{i\pi/d}\notin\mu_d$ are in general necessary.
\end{theorem}
...
$A\gamma\equiv s \pmod d$ produces, constructively, $u=(d/g_i)U_i$ with $uA\equiv 0$
...
\cite[Thm.~4]{paperB}. Finally, an integral gauge exists because the $\mathrm{GF}(2)$
system $Aa\equiv s_\tau$ is solvable: any $\mathbb Z_2$-kernel vector pairs evenly
with $s_\tau$ by the same spectrum theorem, and double annihilation over $\mathbb Z_2$
applies; kernel pairings are gauge-invariant. Certified end-to-end, with random and
pinned positives, in \texttt{solvability\_equivalence.py} (V44).
\end{proof}
...
Thm.~\ref{thm:avn} & this note; \cite{paperB} Thm.~1, 4 & \texttt{solvability\_equivalence.py} (V44)\\
